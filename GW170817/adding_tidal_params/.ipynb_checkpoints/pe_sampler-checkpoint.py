...
def dynesty_sampler(interp_data, boundary, boundary_dL_tc, sampler_params=None, save_to_hdf=False, **params):
    ...
    sample_keys = ['Mc', 'mass_ratio', 'chi1z', 'chi2z', 'lambda1', 'lambda2', 'ra', 'dec', 'iota', 'pol', 'dL', 'tc']
...
def generate_postSamps(raw_samples, save_to_hdf=False):
    ...
    lambda1, lambda2 = raw_samples['lambda1'], raw_samples['lambda2']
    ...
    samples = np.zeros((len(mchirp), 12))
    samples[:,0], samples[:,1], samples[:,2], samples[:,3], samples[:,4], samples[:,5] = mchirp, q, chi1z, chi2z, lambda1, lambda2
    samples[:,6], samples[:,7], samples[:,8], samples[:,9], samples[:,10], samples[:,11] = iota, ra, dec, distance, pol, tc
    ...
    params = ['Mc', 'mass_ratio', 'chi_eff', 'lambda1', 'lambda2', 'iota', 'ra', 'dec', 'dL']
    ...
    eff_samps_dict.update(dict(pol=eff_samps[:,10], tc=eff_samps[:,11], eta=eta, chi1z=eff_samps[:,2], chi2z=eff_samps[:,3], seed_PE=seed_PE, nodes_seed=nodes_seed))
            
    if(save_to_hdf):
        
        params = ['Mc', 'mass_ratio', 'chi1z', 'chi2z', 'chi_eff', 'lambda1', 'lambda2', 'iota', 'ra', 'dec', 'dL']
...
def generate_postSamps_pycbc(raw_samples, save_to_hdf=False):
    ...
    eff_samps_dict.update(dict(pol=eff_samps[:,10], tc=eff_samps[:,11], eta=eta, chi1z=eff_samps[:,2], chi2z=eff_samps[:,3], seed_PE=seed_PE))
