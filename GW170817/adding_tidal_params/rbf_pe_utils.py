...
def generate_nodes(event, fiducial_params, nodes_params, ifos, fLow):
    ...
    lambda1_min, lambda1_max = boundary['lambda1']
    lambda2_min, lambda2_max = boundary['lambda2']
    ...
    if(nodes_params['nodes_gauss_num'] != 0):
        ...
        lambda1 = (lambda1_max - lambda1_min)*np.random.rand(int(nodes_params['Nnodes']))+ lambda1_min
        lambda2 = (lambda2_max - lambda2_min)*np.random.rand(int(nodes_params['Nnodes']))+ lambda2_min
        ...
        nodes = np.column_stack((theta0, theta3, temp[:,2], temp[:,3], lambda1, lambda2))
        ...
        nodes = np.column_stack((theta0, theta3, s1z, s2z, lambda1, lambda2))
...
def sigmasq_zc_calc(theta, psd, data, waveform_params):
    ...
    theta0, theta3, s1z, s2z, lambda1, lambda2 = theta
    ...
    htilde, _ = get_fd_waveform(approximant=waveform_params['approximant'], mass1=m1, mass2=m2, spin1z=s1z, spin2z=s2z, lambda1=lambda1, lambda2=lambda2, \
...
def RBFInterpolatedLikelihood(q, interpolants, basis_vectors, times, **params):
    ...
    mchirp, mass_ratio, s1z, s2z, lambda1, lambda2, ra, dec, iota, pol, distance, tc = q 
    ...
    q_int = np.array([theta0, theta3, s1z, s2z, lambda1, lambda2]).reshape(1,6)
...
def prior_transform(cube, boundary, boundary_dL_tc):
    ...
    cube[4] = boundary['lambda1'].min() + (boundary['lambda1'].max() - boundary['lambda1'].min()) * cube[4]
    cube[5] = boundary['lambda2'].min() + (boundary['lambda2'].max() - boundary['lambda2'].min()) * cube[5]
    cube[6] = 2*np.pi*cube[6]              # ra: uniform sky
    cube[7] = np.arcsin(2*cube[7] - 1)     # dec: uniform sky
    cube[8] = np.arccos(2*cube[8] - 1)     # iota: sin angle
    cube[9] = 2*np.pi*cube[9]              # pol: uniform angle
    cube[10] = np.power((boundary_dL_tc['dL'].max()**3 - boundary_dL_tc['dL'].min()**3)*cube[10] + boundary_dL_tc['dL'].min()**3, 1./3) # distance: unifrom prior in dL**3
    cube[11] = boundary_dL_tc['tc'].min() + (boundary_dL_tc['tc'].max() - boundary_dL_tc['tc'].min()) * cube[11]                   # tc: uniform prior
...
def pycbc_log_likelihood(q, model):
    ...
    model.update(mass1=m1, mass2=m2, spin1z=s1z, spin2z=s2z, lambda1=lambda1, lambda2=lambda2, tc=tc, distance=distance, ra=ra, dec=dec, inclination=iota, polarization=pol)
