...
def pos_samples(filename):
    ...
    lambda1, lambda2 = np.array(file['lambda1']), np.array(file['lambda2'])
    iota, ra, dec = np.array(file['iota']), np.array(file['ra']), np.array(file['dec'])
    dL =  np.array(file['dL'])
        
    return np.column_stack((mchirp, q, chi_eff, lambda1, lambda2, iota, ra, dec, dL))
...
def title_formats(samps, labels, titles, fmt_arr, measure=None, bins=None, flag=None):
    ...
    range_vals = []
    tlabels = np.empty(samps.shape[1], dtype=object)
