...
def dynesty_sampler(interp_data, boundary, boundary_dL_tc, sampler_params=None, save_to_hdf=False, **params):
    ...
    with open('seeds_and_timings_rbf.txt', 'a') as f:
        ...
        f.write('\n' + 'number of likelihood calls: %d, number of raw samples: %d '%(result['ncall'].sum(), result['samples'].shape[0]))
...
def dynesty_sampler_pycbc(model, boundary, boundary_dL_tc, sampler_params=None, save_to_hdf=False):
    ...
    with open('seeds_and_timings_pycbc.txt', 'a') as f:
        ...
        f.write('number of likelihood calls: %d, number of raw samples: %d '%(result['ncall'].sum(), result['samples'].shape[0]))
...
def generate_postSamps(raw_samples, save_to_hdf=False):
    ...
    with open('seeds_and_timings_rbf.txt', 'a') as f:
        ...
        f.write('\n' + 'number of effective samples: %d'%(effective_size))
