...
nLive_list = [3000, 3500]
nodes_seed = 9
seed_PE = 0

for nLive in nLive_list:
    ...
    mchirp_min, mchirp_max = mchirp_sol[-1] - 0.0002, mchirp_sol[-1] + 0.0002
    ...
    with open('seeds_and_timings_rbf.txt', 'a') as f:
        ...
        f.write('\n' + 'nodes_seed: %d, start_up_time: %.2f minutes'%(nodes_seed, (et_start_up-st_start_up)/60))
    ...
    eff_samps_dict = generate_postSamps(raw_samples, nLive, save_to_hdf=True)
    et_PE = time.time()

    with open('seeds_and_timings_rbf.txt', 'a') as f:

        f.write('\n' + 'nLive: %s, rbf_PE_seed: %s, rbf_PE_time: %.2f minutes'%(nLive, seed_PE, (et_PE-st_PE)/60))

print('sampling at 10 PE seeds completed......')
