...
analysis_start_time = 68
analysis_end_time = 12
sampling_frequency = 4096 
low_frequency_cutoff = 20
high_frequency_cutoff = 1024 #512
high_pass_cutoff = 18
seg_len = 2
filter_order = 4
...
if any(choices):
    ...
    data, psd = get_cleaned_data_psd(event, ifos, sampling_frequency, analysis_start_time, analysis_end_time, high_pass_cutoff, \
...
# center from the trigger and calculate the covarince matrix at the center
...
eta_fid = eta_from_mass1_mass2(m1_fid, m2_fid)
mass_ratio_fid = m1_fid / m2_fid

fiducial_params['Mc'] = Mc_fid
fiducial_params['eta'] = eta_fid
...
interp_data = start_up(event, fiducial_params, data_dict, waveform_params, rbf_params, nodes_params, \
                                               ifos, nProcs, savetrainingdata=False, saveinterpolants=True)
...
with open('seeds_and_timings.txt', 'a') as f:
    
    f.write('nodes_seed: %d, nodes_gauss_num: %.2f, start_up_time: %.2f minutes'%(nodes_seed, nodes_gauss_num, (et_start_up-st_start_up)/60))
